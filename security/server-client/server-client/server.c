...
{
perror("Can’t  set the local certificate from CertFile");
abort();
}
/* set the private key from KeyFile (may be the same as CertFile) */
if ( SSL_CTX_use_PrivateKey_file(ctx, KeyFile, SSL_FILETYPE_PEM) !=1 )
{
perror("Can’t  set the private key from KeyFile");
abort();
}
/* verify private key */
...
char buf[1024] = {0};
int sd, bytes;
const char* ServerResponse="<\\Body>\\<Name>aticleworld.com</Name>\\<year>1.5</year>\\<BlogType>Embedede_and_cc++<\\BlogType>\\<Author>amlendra<Author>\\<\\Body>";
...
SSL *ssl;
 ...
 cli = accept(server, (SA*)&address, &len); /* accept connection as usual */
printf("Connection: %s:%d\n",inet_ntoa(address.sin_addr), ntohs(address.sin_port));
ssl = SSL_new(ctx); /* get new SSL state with context */
SSL_set_fd(ssl, cli); /* set connection socket to SSL state */
